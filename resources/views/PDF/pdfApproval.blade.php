...
<div id="project" >
  ...
  <div style="font-size: 15px;"> Supplier: {{$Invoice[0]->HR->name}}</div>
  <span>
      <div> {{$Invoice[0]->HR->Adress}}</div>
      <div>{{$Invoice[0]->HR->contact}}</div>
      </span>
</div>
@if($Invoice[0]->statusPaid==0)
 ...
 <span style="color: red; border:1px solid red; border-radius: 10% ;width: 200px;
	height: 200px; font-size: 19px; ">UNPAID</span>
        ...
        <div style="font-size: 15px;">Created at: {{$Invoice[0]->Staffs[0]->created_at}}</div>
      ...
      <table style="padding-top: 100px;">
        ...
        @php
          $i=0;
        @endphp
        @foreach($Invoice[0]->Staffs as $value)
          <tr>
        ...
        @if($Invoice[0]->statusPaid==0)
          ...
          <tr>
            ...
            <td class="total"> {{($Invoice[0]->HR->total_balance-$Invoice[0]->HR->paid)}}</td>
          </tr>
          <tr>
            <td colspan="5" class="grand total" style="text-align: right;">GRAND TOTAL</td>
            <td class="grand total"> {{($Invoice[0]->HR->total_balance-$Invoice[0]->HR->paid)}}</td>
          </tr>
        @else
          <tr>
            <td  class="sum" colspan="5">SUBTOTAL</td>
          ...
          <tr>
            <td  class="sum" colspan="5">Previous Due</td>
              <td class="total"> {{($Invoice[0]->HR->total_balance-($Invoice[0]->HR->paid-$paymenthist)-$Invoice[0]->price)}}</td>
          </tr>
          <tr>
            <td colspan="5" class="grand total" style="text-align: right;">GRAND TOTAL</td>
              <td class="grand total"> {{$Invoice[0]->HR->total_balance-$Invoice[0]->HR->paid}}</td>
