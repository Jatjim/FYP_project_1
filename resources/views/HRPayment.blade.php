...
@section('content')
    ...
    <script>
        var HRID=0;
        var paidOrginal=0;
        function getStaffs(r,x,price,IDsupp) {
            document.getElementById('display_box').innerHTML = 0;

            document.getElementById('display_HR').innerHTML = "Not found"
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $.each(response, function (i, data) {
                        ...
                        document.getElementById('display_HR').innerHTML = data.HR.name;
                        document.getElementById('display_bal').innerHTML = data.HR.total_balance;
                        document.getElementById('display_paid').innerHTML = data.HR.paid;
                    ...
                    if(paidOrginal!=0) {
                        var unpayBtn = '<button class="btn btn-warning btn-sm" data-toggle="modal" data-target="#addPayment" onclick="getStaffs(' + r + ',' + 2 + ',' + price + ',' + IDsupp + ')"><i class="fa fa-minus"></i> Unpay invoice</button>';
    ...
    <div class="row">
        <div class="col-md-7">
            <div class="box  box-solid">
                ...
                <!-- /.box-header -->
                <div class="box-body">
                    <table  id="StaffsTBL" class="table table-bordered table-hover">
                        <thead>
                        <tr>
                        ...
                        @foreach ($Approval as $Approval)
                            <tr>
                                @if($Approval->statusPaid==-1)
                                    <td><label class="label label-danger">unpaid</label></td>
                                @else
                                    <td><label class="label label-success">paid</label></td>

                                @endif
                                <td>{{ $Approval->boxID}}</td>

                                <td>{{ $Approval->price}}</td>
                                <td>{{ $Approval->HR->name}}</td>


                                <td>
                                    @if($Approval->statusPaid==-1)
                                        <button class="btn btn-primary btn-sm" onclick="getStaffs({{$Approval->boxID}},1,{{$Approval->price}},{{$Approval->HR->id}})"><i class="fa fa-eye-slash"></i> Details</button>
                                        <button class="btn btn-danger btn-sm" data-toggle="modal" data-target="#addPayment" onclick="getStaffs({{$Approval->boxID}},2,{{$Approval->price}},{{$Approval->HR->id}})"><i class="fa fa-plus-circle"></i> Pay</button>
                                        <a href="{{URL::to('/makepdfpurchase/'.$Approval->boxID)}}" target="_blank" class="btn btn-info btn-sm" ><i class="fa fa-print"></i></a>

                                    @else
                                        <button class="btn btn-primary btn-sm" onclick="getStaffs({{$Approval->boxID}},1,{{$Approval->price}},{{$Approval->HR->id}})"><i class="fa fa-eye-slash"></i> Details</button>
                                        <button class="btn btn-danger btn-sm" data-toggle="modal" data-target="#addPayment" onclick="getStaffs({{$Approval->boxID}},2,{{$Approval->price}},{{$Approval->HR->id}})" disabled><i class="fa fa-plus-circle"></i> Pay</button>
                                        <a href="{{URL::to('/makepdfpurchase/'.$Approval->boxID)}}" target="_blank"  class="btn btn-info btn-sm" ><i class="fa fa-print"></i></a>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="payToHR();">Pay</button>
                </div>
            </div>
            <!-- /.modal-content -->
        </div>
